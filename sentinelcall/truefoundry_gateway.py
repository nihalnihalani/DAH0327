...
# Map Claude model names to OpenAI equivalents for direct OpenAI fallback.
OPENAI_MODEL_MAP = {
...
class TrueFoundryGateway:
    ...
    def _resolve_model_name(self, model: str) -> str:
        ...
        we can pass raw model names directly. When using OpenAI directly,
        Claude model names are mapped to OpenAI equivalents.
        """
        if self._mode == "openai":
            return OPENAI_MODEL_MAP.get(model, "gpt-4o-mini")
        return model
    ...
    def get_gateway_config(self) -> dict:
        ...
        This shows how Pager0 would configure TrueFoundry's
        priority-based routing for model escalation and fallback.
        """
        provider = self._provider_name
        return {
            "load_balancing": {
                "name": "pager0-model-escalation",
                ...
                "rules": [
                    {
                        ...
                        "when": {
                            "metadata": {
                                "pager0_severity": {"condition": "exists"}
                            }
                        },
                        "strategy": "priority",
